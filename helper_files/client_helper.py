...
def get_latest_price(ticker,stock_client:StockHistoricalDataClient):  
   ...
   """  
   try:  
        sbr=StockBarsRequest(symbol_or_symbols=[ticker],timeframe=TimeFrame.Hour,start=(datetime.today()-relativedelta(days=1)))
...
def parse_time_span(span):
    match = re.match(r"(\d+)([a-zA-Z]+)", span)
    if not match:
        raise ValueError(f"Ungültige Zeitspanne: {span}")
...
def filter_dataframe(df, time_span, reference_date=None):
    if reference_date is None:
        reference_date = datetime.now(tz=pytz.UTC)  # Aktuelles Datum mit UTC-Zeitzone
    else:
        reference_date = pd.to_datetime(reference_date).tz_localize("UTC")  # Sicherstellen, dass die Zeitzone "UTC" ist
    
    delta = parse_time_span(time_span)
    start_date = reference_date - delta
    filtered_df = df[(df.index >= start_date) & (df.index <= reference_date)]
    return filtered_df